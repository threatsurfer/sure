...
class UpBankAccount::CategoryMatcherTest < ActiveSupport::TestCase
  setup do
    @family = families(:empty)
    @family.categories.bootstrap! # creates SURE's default categories
    @matcher = UpBankAccount::CategoryMatcher.new(@family.categories.to_a)
  end
  ...
  test "leaves Up-specific categories with no honest default uncategorized" do
    %w[
  ...
  test "does not match when the family has none of the target categories" do
    ...
    matcher = UpBankAccount::CategoryMatcher.new(bare_family.categories.reload.to_a)
